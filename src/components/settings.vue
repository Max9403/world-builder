<template>
  <div class="settings">
    ...
    <div class="columns is-multiline">
      ...
      <div class="column is-half">
        <button class="button is-info is-fullwidth" type="button" @click.prevent="showExport = true">Export</button>
      </div>
      <div class="column is-half">
        <button class="button is-warning is-fullwidth" type="button" @click.prevent="showImport = true">Import</button>
      </div>
    ...
    <modal class="import-content" :show="showImport" @hide="showImport = false">
      <div class="notification" :class="{'is-success': importStatus == 'valid', 'is-warning': importStatus == 'missing', 'is-danger': importStatus == 'invalid'}">
      ...
      <div class="columns">
        <div class="column">
          <button class="button is-warning is-fullwidth" @click.prevent="loadImport" :disabled="importStatus != 'valid'">Import</button>
        </div>
        <div class="column">
          <label class="button is-warning is-fullwidth" :class="{'is-loading': loadingFile}" :disabled="loadingFile">
            Load file
...
<script>
  ...
  export default {
    ...
    data() {
      return { parsed: {settings: this.$store.state.settings, flow: this.$store.state.flow}, currentSettings: Object.assign({tiles: {}}, _.cloneDeep(this.settings)), showExport: false, showImport: false, importStatus: "valid", loadingFile: false };
    },
    computed: {
      exported() {
